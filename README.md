...
│   └── 📁 output/                     # 생성 결과물 예시
│       └── sample_run/                # TS_07 DoIP 공격 시나리오 예시
│           ├── 📁 steps/              # T1~T4 개별 스크립트
...
- `system_model.json` AND `testbed_config.json`: 테스트베드의 네트워크 토폴로지와 엔드포인트
...
> 💡 **실제 예시 확인**: `examples/output/sample_run/` 폴더에서 TS_07 DoIP 공격 시나리오의 실제 생성 결과물을 확인할 수 있습니다.
